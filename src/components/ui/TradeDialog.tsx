...
export const TradeDialog: React.FC<TradeDialogProps> = ({
  ...
  const handleExecute = () => {
    ...
    const basePrice =
      orderType === 'Market'
        ? currentPrice
        : convertValue(limitPrice, user?.currency || 'INR', baseCurrency);
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-surface border border-border rounded-2xl p-6 shadow-modal z-50 animate-in zoom-in-95 duration-200">
          ...
          <div className="space-y-6">
            ...
            <div className="flex p-1 bg-background rounded-xl border border-border">
              <button
                ...
                className={cn(
                  ...
                  tradeType === 'buy'
                    ? 'bg-success text-text-primary shadow-sm'
              ...
              <button
                ...
                className={cn(
                  'flex-1 py-2 rounded-lg text-sm font-bold transition-all',
                  tradeType === 'sell'
                    ? 'bg-danger text-text-primary shadow-sm'
                    : 'text-text-muted hover:text-text-primary'
                )}
              >
                Sell
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-bold font-numbers">
                {formatCurrency(initialLimitPrice, user?.currency)}
              </span>
            </div>
            ...
            <div className="rounded-xl bg-surface-elevated p-4 space-y-3 border border-border/50">
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="font-numbers text-text-primary">
                  {formatCurrency(brokerage, user?.currency)}
                </span>
              </div>
              <div className="h-px bg-border/50" />
              <div className="flex justify-between text-sm font-bold">
                <span>Total Amount</span>
                <span className="font-numbers text-primary">
            ...
            {!showConfirm ? (
              ...
              >
                Review {tradeType.toUpperCase()} Order
              </Button>
            ) : (
              <div className="space-y-3 animate-in fade-in duration-200">
                <div className="p-3 rounded-lg bg-warning/10 border border-warning/30 text-center">
                  ...
                  <p className="text-xs text-text-muted mt-1">
                    {qty} shares of {symbol} at {formatCurrency(displayPrice, user?.currency)} ={' '}
                    {formatCurrency(grandTotal, user?.currency)}
                  </p>
                </div>
                <div className="flex gap-3">
                  ...
                  <Button
                    ...
                    className={cn(
                      'flex-1 py-3 font-bold shadow-lg',
                      tradeType === 'buy'
                        ? 'bg-success hover:bg-success/90 shadow-success/20'
                        : 'bg-danger hover:bg-danger/90 shadow-danger/20'
                    )}
